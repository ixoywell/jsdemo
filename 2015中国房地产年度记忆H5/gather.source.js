...
var config = {
    ...
    gather_param:{'uid':'', 'host':'', 'url':'', 'referer_url':'', 'screen_height':'', 'screen_width':'', 'brower':'',
                  ...
                  'level1_page':'','level2_page':'','custom_id':'','webtype':'','os': '','news_source':'','source_ext':'','ln':''},
    gather_event_param:{'event':'','event_name':'','city':'','weixin_house_id':'','level1_page':'','level2_page':'','source':'',
    					'param1':'','param2':'','param3':'','param4':'','webtype':'','os': '','user_agent':''},
    init:function() {
        if(config.host_name == '') {
...
var gather = {
    ...
    init:function(options) {
        ...
        this.url_param=this.get_param(options);
        var img = new Image();
        img.onload = null;
        img.src = this.url + this.url_param;
    ...
    get_param:function(options) {
        ...
        */
        ...
        param.news_source=self._getNewsSource();
        param.source_ext=self._getSourceExt();
        param.ln=self._getLn();
    ...
    */
    ...
    _getNewsSource:function(){
    	if ( typeof(news_source)!= "undefined" ){
    		return trim(news_source);
    	}
    	return "";
    },
    _getLon:function(){
    ...
    _getSourceExt:function(){
    	source_ext = Request('source_ext');
    	if ( source_ext!='' ){
    		return trim(source_ext);
    	}
    	return "";
    },
    _getLn:function(){
    ...
    _getWebtype:function(){
    	...
    	if(url.indexOf('wx.') > 0 || url.indexOf('://weixin.') > 0 || url.indexOf('/weixin/') > 0 || url.indexOf('site=weixin') > 0 || url.indexOf('wx.leju.com') > 0)
    	{
    		return 'weixin';
    	}
